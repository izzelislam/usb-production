...
class PurchasesExport implements FromCollection, WithHeadings, WithStyles, WithEvents
{
    protected $request;
    ...
    {
        $query = Purchase::query()->with(['vendor', 'items.item']);
        ...
        foreach ($purchases as $purchase) {
            foreach ($purchase->items as $index => $item) {
                ...
                if ($index === 0) {
                    $row->push($purchase->invoice_number);
                    $row->push(\Carbon\Carbon::parse($purchase->date)->format('d/m/Y H:i'));
                } else {
                    $row->push('');
                    $row->push('');
                }
            ...
            AfterSheet::class => function(AfterSheet $event) {
                ...
                while ($currentRow <= $highestRow) {
                    ...
                    if (!empty($kodeNota)) {
                        ...
                        // Find all rows with the same purchase (empty Kode Nota)
