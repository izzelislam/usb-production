...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4 print:hidden">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Detail Pembelian</h1>
            <p class="text-sm text-gray-500 mt-1">Informasi lengkap transaksi pembelian</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden print:shadow-none print:border-none">
        ...
        <div class="p-8 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between gap-6">
            <div>
                ...
                <p class="text-primary-600 font-mono font-medium">{{ $purchase->invoice_number }}</p>
                
                <div class="mt-4 text-sm text-gray-600">
                    <p class="font-semibold text-gray-800">Diterbitkan Untuk:</p>
            ...
            <div class="text-right">
                <div class="mb-4">
                    ...
                    <p class="font-medium text-gray-800">{{ \Carbon\Carbon::parse($purchase->date)->format('d F Y') }}</p>
                </div>
                
                <div>
                    <p class="text-sm text-gray-500 mb-1">Supplier</p>
                    <h3 class="font-bold text-gray-800 text-lg">{{ $purchase->vendor->name }}</h3>
                    <p class="text-sm text-gray-600">{{ $purchase->vendor->address }}</p>
                    <p class="text-sm text-gray-600">{{ $purchase->vendor->phone }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 px-8 py-6 border-t border-gray-100 flex justify-end gap-3 print:hidden">
            ...
            <button type="button" onclick="showDeleteModal('{{ route('purchases.destroy', $purchase) }}', '{{ $purchase->invoice_number }}')" class="inline-flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium shadow-lg shadow-red-600/30">
