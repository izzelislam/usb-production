...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-2">
            <!-- Date Filter Form -->
            <form action="{{ route('reports.payroll') }}" method="GET" id="filterForm" class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-2 ml-2 border-l pl-4 border-gray-300">
                <a href="{{ route('reports.payroll.export', ['start_date' => $startDate, 'end_date' => $endDate]) }}" class="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100 text-xs uppercase text-gray-500 font-semibold">
                        ...
                        <th class="px-6 py-4 text-right">Total Gaji (Rp)</th>
                        <th class="px-6 py-4 text-center">Jumlah Karyawan</th>
                        <th class="px-6 py-4 text-right">Rata-rata Gaji (Rp)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($monthlyPayroll as $data)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 text-sm text-gray-800 font-medium">{{ $data['month'] }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600 text-right">Rp {{ number_format($data['total_salary'], 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600 text-center">{{ $data['total_employees'] }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600 text-right">Rp {{ number_format($data['average_salary'], 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-8 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center">
                                    <i class="ph ph-money text-3xl mb-2 text-gray-400"></i>
                ...
                <tfoot class="bg-gray-50 font-semibold text-gray-800">
                    <tr>
                        ...
                        <td class="px-6 py-4 text-right">Rp {{ number_format($monthlyPayroll->sum('total_salary'), 0, ',', '.') }}</td>
