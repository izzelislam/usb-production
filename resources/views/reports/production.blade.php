...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-2">
            <!-- Date Filter Form -->
            <form action="{{ route('reports.production') }}" method="GET" id="filterForm" class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-2 ml-2 border-l pl-4 border-gray-300">
                <a href="{{ route('reports.production.export', ['start_date' => $startDate, 'end_date' => $endDate]) }}" class="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-100 text-xs uppercase text-gray-500 font-semibold">
                        ...
                        <th class="px-6 py-4 text-right">Total Produksi (Pcs)</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($monthlyData as $data)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 text-right">{{ number_format($data['total_produced'], 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600 text-center">{{ $data['total_items'] }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-8 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center">
                                    <i class="ph ph-calendar-x text-3xl mb-2 text-gray-400"></i>
                                    <p>Tidak ada data produksi pada periode ini.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                ...
                <tfoot class="bg-gray-50 font-semibold text-gray-800">
                    <tr>
                        ...
                        <td class="px-6 py-4 text-right">{{ number_format($monthlyData->sum('total_produced'), 0, ',', '.') }}</td>
                        <td class="px-6 py-4 text-center">{{ $monthlyData->sum('total_items') }}</td>
                    </tr>
                </tfoot>
                @endif
            </table>
