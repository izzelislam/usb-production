...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Kelola data penjualan produk</p>
        </div>
        <a href="{{ route('sales.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors shadow-lg shadow-primary-600/30 font-medium">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div class="md:col-span-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="No. Invoice..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                    </div>
                </div>
                ...
                <div class="md:col-span-2 flex gap-2 items-end">
                    ...
                    <a href="{{ route('sales.index') }}" class="px-4 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-medium w-full text-center" title="Reset">
    ...
    <!-- Bulk Action Form -->
    <form action="{{ route('sales.bulk_action') }}" method="POST" target="_blank">
        @csrf
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-4 border-b border-gray-100 bg-gray-50 flex items-center gap-3">
                ...
                <a href="{{ route('sales.export', request()->all()) }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm font-medium shadow-sm flex items-center gap-2">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <input type="checkbox" name="ids[]" value="{{ $sale->id }}" class="sale-checkbox rounded border-gray-300 text-primary-600 focus:ring-primary-500 w-4 h-4 cursor-pointer">
                            </td>
                            <td class="px-6 py-4">
                                <span class="font-mono font-medium text-primary-600">{{ $sale->invoice_number }}</span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-800">{{ $sale->buyer?->name ?? '-' }}</div>
                                @if($sale->buyer?->phone)
                                    <div class="text-xs text-gray-500">{{ $sale->buyer->phone }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button type="button" onclick="openPrintModal('{{ route('sales.print', $sale) }}')" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition-colors font-medium" title="Cetak Nota">
                                        <i class="ph ph-printer text-lg"></i>
                                        <span>Cetak</span>
                                    </button>
                                    <a href="{{ route('sales.show', $sale) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition-colors font-medium" title="Lihat Detail">
                                        <i class="ph ph-eye text-lg"></i>
                                        <span>Detail</span>
                                    </a>
                                    <button type="button" onclick="showDeleteModal('{{ route('sales.destroy', $sale) }}', '{{ $sale->invoice_number }}')" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors font-medium" title="Hapus">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <a href="{{ route('sales.create') }}" class="text-primary-600 hover:text-primary-700 font-medium">
    ...
    <div x-data="{ show: false, url: '' }"
        ...
        <div x-show="show"
            ...
            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 border-b border-gray-100 flex justify-between items-center">
                <h3 class="text-lg font-semibold leading-6 text-gray-900">Preview Nota Penjualan</h3>
